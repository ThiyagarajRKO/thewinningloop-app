      ...
      <div className="main">
        <header className="topbar">
          <span className="crumb">
            {current?.live ? "Connected" : "Not connected"} /{" "}
            <b>{current?.label}</b>
          </span>
...
function SweepMeta() {
  ...
  return (
    <span className="sweep-meta">
      {fmt(s.stats.ads)} ads · {fmt(s.stats.domains)} stores
    </span>
  );
}
...
function TrendsCard({ term }) {
  ...
  const first = pts[0]?.value ?? 0,
    last = pts[pts.length - 1]?.value ?? 0;
  const delta = first > 0 ? Math.round(((last - first) / first) * 100) : 0;

  const m = data.market;
  const verdictClass =
    m?.verdict?.label === "Opportunity"
      ? "up"
      : m?.verdict?.label === "Saturated"
        ? "down"
        : "";
    ...
    <div className="trends-card">
      ...
      <div className="trends-foot">
        ...
        <span className="mono-dim">
          as of {new Date(data.fetched_at).toLocaleDateString()}
        </span>
      </div>
      {m && (
        <div className="market-verdict">
          <span className={"trends-delta " + verdictClass}>
            {m.verdict.label}
          </span>
          <span className="mono-dim">
            {m.advertisers} advertisers · {m.ads} ads indexed for this term
          </span>
          <p className="detail-desc" style={{ margin: 0, flexBasis: "100%" }}>
            {m.verdict.detail}
          </p>
        </div>
      )}
...
function MagicAI({ onOpenLibrary }) {
  ...
  const load = async (term) => {
    ...
    const adsRes = await fetch(
      `/api/ads?q=${encodeURIComponent(term)}&sort=creatives&limit=60`,
    )
      .then((r) => r.json())
      .catch(() => ({ ads: [] }));
    setAds(adsRes.ads || []);
  ...
  const isSwept =
    submitted && swept.some((s) => s.toLowerCase() === submitted.toLowerCase());
  const hot = (ads || []).filter((a) => a.ads_using_creative >= 5).length;
  const domains = new Set(
    (ads || []).map((a) => a.landing_domain).filter(Boolean),
  );
  ...
  return (
    <>
      <div className="research-intro">
        <h1>Magic AI</h1>
        <p>
          Type a product idea. See who is running ads for it and how the market
          is trending.
        </p>
      </div>
      ...
      <form className="filters research-form" onSubmit={onSubmit} role="search">
        <div className="field" style={{ flex: 1, minWidth: 260 }}>
          ...
          <input
            id="rq"
            placeholder="e.g. posture corrector, led face mask…"
            value={q}
      ...
      {!submitted && (
        <div className="empty">
          ...
          <p>
            Checks the ads already indexed for that keyword and shows Google
          ...
          {loading ? (
            ...
            <>
              <div className="kpis">
                ...
                <Kpi
                  label="Advertisers"
                  value={new Set(ads.map((a) => a.advertiser_handle)).size}
                />
            ...
            <div className="empty">
              ...
              {isSwept ? (
                <p>
                  This keyword was swept but returned nothing — the term may be
                  too narrow, or the sweep found no active ads that day.
                </p>
              ) : (
                <>
                  <p>
                    This keyword hasn&apos;t been swept from the Facebook Ad
                    Library yet. Trends above still show demand even with no ads
                    indexed.
                  </p>
                  <div className="empty-need">
                    <b>Run a sweep for this keyword</b>
                    <code>
                      node scripts/scrape-fb.mjs &quot;{submitted}&quot; US 8 22
                    </code>
                    <code>
                      node scripts/ingest.mjs payloads/&lt;output&gt;.json
                      &quot;{submitted}&quot; US
                    </code>
                  </div>
                </>
...
function SyncModal({ open, onOpenChange, onDone }) {
  ...
  const start = async (e) => {
    ...
    try {
      ...
      pollRef.current = setInterval(async () => {
        const r = await fetch(`/api/sync/${data.jobId}`)
          .then((r) => r.json())
          .catch(() => null);
        if (!r || r.error) return;
        setJob(r);
  ...
  const statusLabel = {
    queued: "Queued…",
    scraping: "Scraping the Ad Library…",
  ...
  return (
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Popup
          className="panel-popup panel-popup-modal sync-modal"
          ...
          {!job ? (
            <form className="sync-form" onSubmit={start}>
              ...
              <p className="sync-hint">
                Scrapes the public Ad Library page live (headless browser, no
                login, no private API) and saves results to your database. Takes
                1-3 minutes.
              </p>
            ...
            <div className="sync-progress">
              ...
              {job.status === "done" && (
                <p className="sync-result">
                  {fmt(job.ads_found)} ads found, {fmt(job.ads_new)} new — saved
                  to your library.
                </p>
              )}
              {job.status === "error" && (
                <p className="panel-range-err">
                  <Icon.alert size={13} />
                  {job.error || "Something went wrong during the sync."}
                </p>
              )}
  ...
  return (
    ...
    >
      <div className="card-media">
        {ad.creative_video ? (
          <>
            ...
            <video
              src={ad.creative_video}
              poster={
                ad.creative_video_poster || ad.creative_image || undefined
              }
              width="258"
              height="258"
              muted
              ...
              loop
            />
            <span className="media-badge" aria-label="Video ad">
              <Icon.play size={11} />
              Video
            </span>
          </>
        ...
        <span
          ...
          title={`${scale} ads reuse this creative`}
        >
        ...
        {ad.ad_details_url && (
          <a
            className="fb-badge"
            href={ad.ad_details_url}
            target="_blank"
            rel="noreferrer"
            title="Open on Facebook"
            aria-label="Open this ad on Facebook"
          >
            <Icon.link size={11} />
          </a>
        )}
      ...
      <div className="card-b">
        <h3 className="card-adv" title={ad.advertiser_name}>
          {ad.advertiser_name || "Unknown advertiser"}
        </h3>
        {ad.body && <p className="card-body">{ad.body}</p>}
        <div className="card-f">
          {ad.landing_domain && (
            <span className="card-dom" title={ad.landing_domain}>
              {ad.landing_domain}
            </span>
          )}
...
function NoMatch() {
  return (
    <div className="empty">
      ...
      <p>
        Widen the search, or run a new sweep to pull fresh ads into the index.
      </p>
      <div className="empty-need">
        <b>Run a sweep</b>
        <code>node scripts/sweep-url.mjs &quot;your product&quot; US</code>
        <code>
          node scripts/ingest.mjs payloads/out.json &quot;your product&quot; US
        </code>
      </div>
    </div>
  );
...
function Stores({ mode }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const r = await fetch("/api/stores?" + new URLSearchParams(q ? { q } : {}))
      .then((r) => r.json())
      .catch(() => ({ stores: [] }));
    setRows(r.stores || []);
    setLoading(false);
  ...
  const saturation = (r) =>
    r.advertisers >= 3 ? "hot" : r.advertisers === 2 ? "warm" : "cool";
  const satLabel = (r) =>
    r.advertisers >= 3
      ? "Saturated"
      : r.advertisers === 2
  ...
  return (
    <>
      <div className="kpis">
        ...
        <Kpi
          label="Saturated"
          value={rows.filter((r) => r.advertisers >= 3).length}
          hint="3+ sellers"
        />
        <Kpi
          label="Open lanes"
          value={rows.filter((r) => r.advertisers === 1).length}
          hint="single seller"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="empty">
          ...
          <h2>No stores yet</h2>
          <p>
            Stores are derived from the landing domains of ads you have already
            swept.
          </p>
        </div>
      ) : (
                ...
                <tr key={r.domain}>
                  ...
                  <td>
                    <div
                      className="bar"
                      style={{
                        width: `${Math.max(4, (r.creatives / max) * 100)}%`,
                      }}
                      title={`${r.creatives} creatives`}
                    />
                  </td>
                  {mode === "saturation" ? (
                    <>
                      ...
                      <td>
                        <span className={`sat ${saturation(r)}`}>
                          <i />
                          {satLabel(r)}
                        </span>
                      </td>
                    ...
                    <>
                      ...
                      >
                        {(r.countries || []).filter(Boolean).join(" ")}
                      </td>
                      <td
                        style={{
                          fontFamily: "var(--mono)",
...
function TableSkeleton() {
  return (
    ...
    >
      {Array.from({ length: 8 }, (_, i) => (
        <div className="sk-line" key={i} style={{ margin: "14px 0" }} />
      ))}
    </div>
  );
}
...
function BrandTracker() {
  ...
  useEffect(() => {
    const pending = brands.some(
      (b) => b.status === "queued" || b.status === "fetching",
    );
    clearInterval(pollRef.current);
  ...
  return (
    <>
      <div className="research-intro">
        <h1>Brand Tracker</h1>
        <p>
          Track a store: products, theme, tracking pixels, contact email, and
          its Facebook/Meta ads — pulled from the store's own public pages,
          saved so you don't re-fetch it every time.
        </p>
      </div>
      ...
      <form className="filters" onSubmit={track} role="search">
        <div className="field" style={{ flex: 1, minWidth: 260 }}>
          ...
          <input
            id="brand-domain"
            placeholder="e.g. alynnewear.com"
            value={domainInput}
        ...
        <button
          className="btn"
          type="submit"
          disabled={starting || !domainInput.trim()}
        >
          {starting ? "Starting…" : "Track brand"}
        </button>
      </form>
      {err && (
        <p className="panel-range-err" style={{ marginBottom: "var(--s4)" }}>
          <Icon.alert size={13} />
          {err}
        </p>
      )}
      ...
      {loading ? (
        ...
        <div className="empty">
          <EmptyGlyph />
          <h2>No brands tracked yet</h2>
          <p>
            Enter a store domain above — takes 20-60 seconds to pull products,
            theme, pixels, and Meta ads.
          </p>
        </div>
      ) : (
        <div className="brand-grid">
          {brands.map((b) => (
            <div key={b.domain} className="brand-card">
              <div className="brand-card-head">
                ...
                >
                  {b.status === "queued" || b.status === "fetching" ? (
                    <span className="row-spinner" style={{ marginRight: 4 }} />
                  ) : null}
                ...
                <>
                  <div className="brand-stats">
                    ...
                    <span>
                      ...
                      {b.fb_active_ads ?? "—"} active Meta ads
                    </span>
                  </div>
                  {b.pixels?.length > 0 && (
                    <div className="brand-pixels">
                      {b.pixels.map((p) => (
                        <span key={p} className="pixel-pill">
                          {PIXEL_LABELS[p] || p}
                        </span>
                      ))}
                    </div>
              ...
              {b.status === "error" && (
                <p className="panel-range-err" style={{ margin: 0 }}>
                  <Icon.alert size={13} />
                  {b.error}
                </p>
              )}
...
const TREND_TIMEFRAMES = [
  { v: "now 7-d", label: "Past 7 days" },
  { v: "today 1-m", label: "Past month" },
  { v: "today 3-m", label: "Past 3 months" },
  { v: "today 12-m", label: "Past 12 months" },
];
...
function TrendsPage() {
  ...
  const loadTrending = async (g) => {
    ...
    const d = await fetch(`/api/trends/daily?geo=${g || "US"}`)
      .then((r) => r.json())
      .catch(() => null);
    if (!d?.available) {
      setErr(d?.error || "Trending unavailable right now.");
      setRows([]);
    } else {
      setRows(toTrendingRows(d.items));
      setErr(null);
    }
    setLoading(false);
  };
  ...
  const runSearch = async (t, g, tf) => {
    ...
    const [termRes, relatedRes] = await Promise.all([
      fetch(
        `/api/trends?${new URLSearchParams({ q: t, geo: g, timeframe: tf })}`,
      )
        .then((r) => r.json())
        .catch(() => null),
      fetch(
        `/api/trends/related?${new URLSearchParams({ q: t, geo: g, timeframe: tf })}`,
      )
        .then((r) => r.json())
        .catch(() => null),
    ]);
  ...
  const openRow = async (row) => {
    ...
    const d = await fetch(
      `/api/trends/daily/detail?${new URLSearchParams({ q: row.query, geo })}`,
    )
      .then((r) => r.json())
      .catch(() => ({ available: false, error: "Request failed." }));
    ...
    <>
      <div className="research-intro research-intro-row">
        <div>
          <h1>Google Trends</h1>
          <p>
            Live search-interest data, fetched directly from Google. Search a
            term, or check what&apos;s trending in shopping right now.
          </p>
        </div>
      ...
      <div className="filters research-form" role="search">
        <div className="field" style={{ flex: 1, minWidth: 220 }}>
          ...
          <input
            id="tq"
            placeholder="Search any term — e.g. air fryer, cold plunge…"
            value={q}
            onChange={(e) => setQ(e.target.value)}
            style={{ width: "100%" }}
            autoFocus
          ...
          {loading && mode === "search" && (
            <span
              className="row-spinner field-spinner"
        ...
        <select
          ...
          onChange={(e) => {
            ...
            if (mode === "trending") loadTrending(e.target.value);
          }}
        >
      ...
      {mode === "search" && termData && !loading && (
        <div className="trends-card">
          <div className="trends-head">
            ...
            <span className="trends-title">
              &quot;{termData.term}&quot; ·{" "}
              {termData.geo === "worldwide" ? "Worldwide" : termData.geo} ·{" "}
              {timeframe}
            </span>
            <span className={"trends-delta " + (delta >= 0 ? "up" : "down")}>
              {delta >= 0 ? "+" : ""}
          ...
          <div className="trends-foot">
            <span>
              avg interest {termData.avg_interest} · peak {peak?.value ?? "—"} (
              {peak?.date})
            </span>
          ...
          {termData.market && (
            <div className="market-verdict">
              <span
                className={
                  "trends-delta " +
                  (termData.market.verdict.label === "Opportunity"
                    ? "up"
                    : termData.market.verdict.label === "Saturated"
                      ? "down"
                      : "")
                }
              >
              ...
              <span className="mono-dim">
                {termData.market.advertisers} advertisers ·{" "}
                {termData.market.ads} ads indexed
              </span>
              <p
                className="detail-desc"
                style={{ margin: 0, flexBasis: "100%" }}
              >
                {termData.market.verdict.detail}
              </p>
            </div>
          )}
        </div>
      ...
      {mode !== "idle" && (
        <div
          ...
          style={{ marginTop: mode === "search" ? "var(--s4)" : 0 }}
        >
          <h2>
            {mode === "trending"
              ? "Trending shopping today"
              : `Related to "${term}"`}
          </h2>
      ...
      {mode === "trending" && wasGlobalFallback && !loading && (
        <p className="panel-range-err" style={{ marginBottom: "var(--s3)" }}>
          <Icon.alert size={13} />
          Google&apos;s trending feed has no worldwide edition — showing United
          States, the same list &quot;Worldwide&quot; would silently fall back
          to here.
        </p>
      )}
      ...
      {mode === "idle" ? (
        <div className="empty">
          <EmptyGlyph />
          <h2>
            Search any term above, or see what&apos;s trending in shopping right
            now
          </h2>
          <p>
            Nothing loads until you ask — this fetches live from Google on
        ...
        <div className="empty">
          ...
          <p>
            No {mode === "trending" ? "trending topics" : "related terms"}{" "}
            returned for this region.
          </p>
        </div>
      ) : (
        <>
            ...
            <table className="table">
              <thead>
                <tr>
                  <th>Query</th>
                  <th>Interest</th>
                  <th style={{ textAlign: "right" }}>Advertisers</th>
                  <th />
                </tr>
              </thead>
              <tbody>
      ...
      <TrendDetailPanel
        ...
        geo={geo}
        detail={panelRow ? rowDetail[panelRow.query] : null}
        onClose={closePanel}
      />
    </>
  );
  ...
  return (
    ...
    >
      <td>
        {row.query}
        {row.swept === true && (
          <span className="tag-swept" style={{ marginLeft: 8 }}>
            indexed
          </span>
        )}
      </td>
      ...
      <td style={{ width: 20 }}>
        {isLoading ? (
          <span className="row-spinner" aria-label="Loading" />
        ) : (
          <Icon.search size={12} style={{ opacity: 0.5 }} />
        )}
      </td>
    </tr>
  );
...
function TrendDetailPanel({ row, loading, detail, geo, onClose }) {
  ...
  const applyRange = async () => {
    ...
    setRangeErr(null);
    const d = await fetch(
      `/api/trends/daily/detail?${new URLSearchParams({ q: row.query, geo, from, to })}`,
    )
      .then((r) => r.json())
      .catch(() => ({ available: false, error: "Request failed." }));
  ...
  const toggleExpanded = () => {
    ...
    if (next && !regions && row) {
      ...
      fetch(
        `/api/trends/daily/regions?${new URLSearchParams({ q: row.query, geo })}`,
      )
        .then((r) => r.json())
        .then(setRegions)
        .catch(() => setRegions({ available: false }))
  ...
  const toggleAds = () => {
    ...
    if (next && !ads && row) {
      setAdsLoading(true);
      fetch(
        `/api/ads?${new URLSearchParams({ q: row.query, limit: "8", sort: "creatives" })}`,
      )
        .then((r) => r.json())
        .then((d) => setAds(d.ads || []))
        .catch(() => setAds([]))
  ...
  const chartData = (active?.points || []).map((p) => ({
    ...
    label: p.date
      .replace(/, \d{4} at (\d{1,2}):00 ([AP]M)/, " $1$2")
      .replace(/, \d{4}$/, ""),
    fullDate: p.date,
    value: p.value,
  }));
  ...
  return (
    ...
    >
      <Dialog.Portal>
        <Dialog.Backdrop className="panel-backdrop" />
        <Dialog.Popup
          className={"panel-popup" + (expanded ? " panel-popup-modal" : "")}
          aria-describedby={undefined}
        >
          {row && (
            <>
              ...
              <div className="panel-stats">
                <div className="panel-stat">
                  <span className="kpi-v" style={{ fontSize: 20 }}>
                    {row.interest}
                  </span>
                ...
                {Number(row.advertisers) > 0 ? (
                  <button
                    type="button"
                    className="panel-stat panel-stat-clickable"
                    onClick={toggleAds}
                  >
                    <span className="kpi-v" style={{ fontSize: 20 }}>
                      {row.advertisers}
              ...
              {adsOpen && (
                <div className="panel-block">
                  <b className="mono-dim">
                    Ads matching &quot;{row.query}&quot;
                  </b>
                  {adsLoading ? (
                    <div className="sk-line" />
                  ) : !ads?.length ? (
                    <p className="detail-desc" style={{ margin: 0 }}>
                      No indexed ads found for this term.
                    </p>
                  ) : (
                    <div className="panel-ads-list">
                      {ads.map((a) => (
                        <Link
                          key={a.library_id}
                          href={`/ad/${a.library_id}`}
                          className="panel-ad-row"
                        >
                          {a.creative_image ? (
                            <img
                              src={a.creative_image}
                              alt=""
                              width={36}
                              height={36}
                            />
                            ...
                            <span
                              className="panel-ad-thumb-empty"
                              aria-hidden="true"
                            />
                          )}
                          <span className="panel-ad-info">
                            <span className="panel-ad-adv">
                              {a.advertiser_name || "Unknown advertiser"}
                            </span>
                            <span className="mono-dim">
                              {a.landing_domain || a.display_domain || "—"}
                            </span>
                          </span>
                          <Icon.link
                            size={12}
              ...
              <div
                className={
                  "panel-chart-wrap" + (expanded ? " panel-chart-wrap-lg" : "")
                }
              >
                {isLoadingChart ? (
                  <div className="panel-chart-loading" aria-busy="true">
                    <span
                      className="row-spinner row-spinner-lg"
                      aria-hidden="true"
                    />
                  ...
                  <p className="detail-desc" style={{ margin: 0 }}>
                    {active?.error || "No trend data for this term."}
                  </p>
                ) : chartData.length === 0 ? (
                  <p className="detail-desc" style={{ margin: 0 }}>
              ...
              {expanded && (
                <div className="panel-block">
                  ...
                  {regionsLoading ? (
                    ...
                    <p className="detail-desc" style={{ margin: 0 }}>
                      {regions?.error || "Regional data unavailable."}
                    </p>
                  ) : (
                        ...
                        <div className="region-row" key={r.code}>
                          ...
                          <div className="region-bar-track">
                            <div
                              ...
                              style={{
                                width: `${Math.max(4, (r.value / regionMax) * 100)}%`,
                              }}
                            />
                          </div>
                          <span className="mono-dim region-value">
                            {r.value}
                          </span>
                        </div>
                      ))}
...
function Boards() {
  return (
    <div className="empty">
      ...
      <p>
        Boards group ads you want to come back to. The tables exist and are
        wired; nothing has been saved into them.
      </p>
      <div className="empty-need">
        <b>Schema ready</b>
...
function NotConnected({ item }) {
  ...
  return (
    <div className="empty">
      <div style={{ color: "var(--text-muted)", marginBottom: "var(--s4)" }}>
        <I size={28} />
      </div>
      <h2>{item.label} is not connected</h2>
      <p>
        This feature has no data source wired up yet. Nothing here is simulated
        — the panel stays empty until a real source is connected.
      </p>
      <div className="empty-need">
        <b>What it needs</b>
        <code>{item.need}</code>
...
function EmptyGlyph() {
  return (
    <svg
      ...
      viewBox="0 0 48 48"
      aria-hidden="true"
      style={{ marginBottom: "var(--s4)", opacity: 0.9 }}
    >
      <circle
        cx="24"
        cy="24"
